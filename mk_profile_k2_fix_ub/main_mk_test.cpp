...
int main() {
    cout << "=== MK-TFHE Time Profiling (k=2) FIXED UB ===" << endl;
    ...
    cout << "Generating Keys (DUMMY)..." << endl;
    vector<bbii::MKSecretKey*> sks(k); 
    bbii::MKBootstrappingKey* bk = new bbii::MKBootstrappingKey(k, mp->n_per_party, mp->get_tfhe_params());
    
    // DUMMY: Skip actual keygen to focus on bootstrapping correctness
    std::cout << "[LOG] Keygen SKIPPED (dummy)" << std::endl;
