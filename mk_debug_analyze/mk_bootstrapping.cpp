...
void mk_blind_rotate(MKRLweSample* acc, const MKLweSample* bk_input, const MKBootstrappingKey* mk_bk, const TFheGateBootstrappingParameterSet* params) {
    std::cout << "DEBUG: Start BlindRotate" << std::endl;
    ...
    for (int u = 0; u < k; ++u) { 
        std::cout << "DEBUG: Loop u=" << u << " (Checking access to bk_fft)" << std::endl;
        // Check vector size access safety
        if (u >= mk_bk->bk_fft.size()) { std::cout << "FATAL: u out of range!" << std::endl; exit(1); }
        
        for (int i = 0; i < n; ++i) { 
            int32_t global_idx = u * n + i;
            // Check array access safety
            // std::cout << "DEBUG: Loop i=" << i << std::endl;
            ...
            // Critical Point: Accessing BK
            if (i >= mk_bk->bk_fft[u].size()) { std::cout << "FATAL: i out of range!" << std::endl; exit(1); }
            mk_cmux(acc, mk_bk->bk_fft[u][i], acc, temp_acc, u, params);
        }
    }
    // delete temp_acc; // Leak
    std::cout << "DEBUG: End BlindRotate" << std::endl;
}

void mk_sample_extract(MKLweSample* output, const MKRLweSample* acc, const LweParams* lwe_params) {
    std::cout << "DEBUG: Start Extract" << std::endl;
    ...
    std::cout << "DEBUG: End Extract" << std::endl;
}
...
void mk_bootstrapping(MKLweSample* res, const MKLweSample* in, const MKBootstrappingKey* bk, Torus32 mu, const TFheGateBootstrappingParameterSet* p) {
    std::cout << "DEBUG: Start Bootstrapping" << std::endl;
    int32_t k=in->k;
    ...
    // delete acc; // Leak
    std::cout << "DEBUG: End Bootstrapping" << std::endl;
}
} 
