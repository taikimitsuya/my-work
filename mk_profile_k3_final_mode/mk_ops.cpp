...
void mk_external_product(MKRLweSample* res, const MKPackedRGSW* bk, const MKRLweSample* acc, int32_t pid, const TFheGateBootstrappingParameterSet* p) {
    ...
    for(int i=0;i<=acc->k;++i){
        ...
        tGswFFTExternMulToTLwe(tmp, bk->sample, const_cast<TGswParams*>(p->tgsw_params));
...
void mk_cmux(MKRLweSample* res, const MKPackedRGSW* bk, const MKRLweSample* in0, const MKRLweSample* in1, int32_t pid, const TFheGateBootstrappingParameterSet* p) {
